    ...
    def after_run(self, run_context, run_values):
        ...
        if (FLAGS.save_predictions_file is not None):
            for idx in range(len(input_ids)):
                ...
                for item_id, input_item_id in zip(predicted_items, input_ids[idx]):
                    try:
                        token = self.vocab.convert_ids_to_tokens([item_id])[0]
                        input_token = self.vocab.convert_ids_to_tokens([input_item_id])[0]
                        score = scores[item_id]
                        output_file.write(f";{token}:{input_token}:{score}")
                    except IndexError:
                        continue
                output_file.write("\n")
